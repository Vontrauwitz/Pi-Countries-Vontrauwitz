  ...
  try {
    ...
    toSavedCountries = newCountries.map(country => {
      return ({
        ...
        //  languages: country.languages  //me traigo el objeto
        //borders
      })
    });
...
router.get('/countries', async (req, res) => {
  ...
  try {
    if (name) {
      ...
      countriesName.length ? res.status(200).send(countriesName) : res.status(200).send(['No countries found']);
    } else {
      await findCountries();
...
router.get('/activity', async (req, res) => {
  ...
  try {
    if (name) {
      ...
      let activityName = await Activity.findAll({
        where: { name: { [Op.iLike]: `%${name}%` } }
      })
      console.log(activityName);
      res.status(200).send(activityName)
    } else {
      res.status(200).send(['No countries found']);
    }
  ...
  //   console.log(id);
  //   try {
  //     id = await Activity.findOne({
  //       where: {
  //         id: id,
  //       }
  //     })
  //     if (!id) {
  //       res.status(200).send("cant find activity to delete")
  //     } else {
  //       await Activity.destroy({
  //         where: {
  //           id: id,
